...
首次使用时，在项目目录打开“终端”并安装依赖：

```bash
python3 -m pip install -r requirements.txt
```

之后可直接双击 `启动WB分析.command` 打开 GUI，也可以在项目目录运行：
...
```bash
chmod +x 启动WB分析.command
```

### Windows

安装 Python 3.10 或更高版本时，请勾选 **Add python.exe to PATH**。下载或解压完整项目后，双击 `start_wb_windows.bat` 即可打开 GUI；脚本会检查 Python、Tkinter 和所需依赖，并在缺少依赖时自动安装。
...
3. 使用图像上方的“当前编辑图像”在内参和目的图像之间切换；两张图的选区互相独立。可在两个“分类名称”输入框中填写如 `GAPDH`、`PDGFRB`，按回车或点击“应用名称”，结果表“类型”列会同步显示。
